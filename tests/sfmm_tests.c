...
Test(sf_memsuite_student, zero_malloc, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(0);
...
Test(sf_memsuite_student, free_header, .signal = SIGABRT,  .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_malloc(2);
	sf_free((char*)sf_mem_start()+40);
}

Test(sf_memsuite_student, free_null, .signal = SIGABRT,  .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_free(NULL);
}

Test(sf_memsuite_student, realloc_smaller, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(double) * 8);//80
	void *z = sf_malloc(10);//32
	void *y = sf_realloc(x, sizeof(int));//32

	cr_assert_not_null(y, "y is NULL!");

	sf_header *hp = (sf_header *)((char*)y - sizeof(sf_header));
	cr_assert(*hp & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((*hp & BLOCK_SIZE_MASK) == 32, "Realloc'ed block size not what was expected!");
	sf_header *zp = (sf_header *)((char*)z - sizeof(sf_header));
	cr_assert((*zp & 0x2) == 0x2, "zp Allocated bit of next block's header is not 1!");
	assert_free_block_count(4048-80-32, 1);
	assert_free_block_count(48, 1);
}

Test(sf_memsuite_student, realloc_same, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(200);
	sf_malloc(10);
	void* y = sf_realloc(x, 199);

	cr_assert_not_null(x, "x is NULL!");
	sf_header *hp = (sf_header *)((char *)x - sizeof(sf_header));
	cr_assert(*hp & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((*hp & BLOCK_SIZE_MASK) == 224, "Realloc'ed block size not what was expected!");
	cr_assert_eq(x, y, "Not the same pointer!");

	assert_free_block_count(4048-224-32, 1);
}

Test(sf_memsuite_student, check_prologue_and_epilogue_with_three_pages_one_list, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(3 * PAGE_SZ - sizeof(sf_prologue) - sizeof(sf_epilogue) - 32 - 16);

	sf_prologue *p = (sf_prologue *)sf_mem_start();
	cr_assert_eq(p->header, 35);
	cr_assert_eq(p->footer, p->header^sf_magic());
	sf_epilogue *e = (sf_epilogue *)sf_mem_end() - 1;
	cr_assert_eq(e->header, 3);
...
Test(sf_memsuite_student, check_prologue_and_epilogue_with_one_page, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(2);
